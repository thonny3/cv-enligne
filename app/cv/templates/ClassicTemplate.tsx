...
export function ClassicTemplate({ data }: { data: CvData }) {
  ...
  return (
    <div className="w-[210mm] min-h-[297mm] bg-white text-slate-800">
      <header className="flex items-center gap-6 px-10 py-8 text-white" style={{ backgroundColor: color }}>
        {info.photo && (
          <div className="h-24 w-24 shrink-0 overflow-hidden rounded-full border-4 border-white/40">
        ...
        <div>
          ...
          {contact && <p className="mt-2 text-xs text-white/80">{contact}</p>}
        </div>
      </header>
